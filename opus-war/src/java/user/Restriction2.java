 ...
 * @author Jetta
 */
@WebServlet(name = "Restriction2", urlPatterns = {"/Restriction2"})
public class Restriction2 extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (request.getParameterValues("ajout") != null) {
                us.ajoutrestriction(request.getParameterValues("ajout"), rolefinale, "ACT000001", direc);
            }
            if (request.getParameterValues("modif") != null) {
                us.ajoutrestriction(request.getParameterValues("modif"), rolefinale, "ACT000002", direc);
            }
            if (request.getParameterValues("suppr") != null) {
                us.ajoutrestriction(request.getParameterValues("suppr"), rolefinale, "ACT000003", direc);
            }
            if (request.getParameterValues("read") != null) {
                us.ajoutrestriction(request.getParameterValues("read"), rolefinale, "ACT000004", direc);
            }
            ...
            request.getRequestDispatcher("/pages/module.jsp?but=utilisateur/restriction.jsp").forward(request, response);
        } catch (Exception ex) {
            ex.printStackTrace();
