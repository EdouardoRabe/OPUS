...
async function transcriptionMessage(demande){
    ...
    return $.ajax({
        url: baseUrl+"/opus/query-generator",
        type: "POST",
        data: formData,
        processData: false,
        contentType: false,
...
async function responseHandler(){
    return $.ajax({
        url: baseUrl+"/opus/response-generator",
        type: "POST",
        dataType: 'json',
        data : {
            nomTable: nomTable,
...
async function resendMessage(){
    return $.ajax({
        url: baseUrl+"/opus/response-formattor",
        type: "GET",
...
function firstMessage(){
    ...
    $('<div class="message new"><figure class="avatar"><img src="/opus/assets/img/logo_A.png" /></figure>' + "Bienvenue sur Async, je suis votre assistant virtuel, comment puis-je vous aider" + '</div>').appendTo($('.mCSB_container')).addClass('new');
    setDate();
    updateScrollbar();
}
...
function realMessage() {
    ...
    $('<div class="message loadingChat new"><figure class="avatar"><img src="/opus/assets/img/logo_A.png" /></figure><span></span></div>').appendTo($('.mCSB_container'));
    updateScrollbar();

    setTimeout(function() {
        $('.message.loadingChat').remove();
        if(valiny.value != null){
            $('<div class="message new"><figure class="avatar"><img src="/opus/assets/img/logo_A.png" /></figure>' + valiny.value + '</div>').appendTo($('.mCSB_container')).addClass('new');
            setDate();
            updateScrollbar();
        ...
        if (valiny.url && valiny.url !== "")  {
            ...
            console.log("urll "+valiny.url);
            $('<div class="message new"><figure class="avatar"><img src="/opus/assets/img/logo_A.png" /></figure>'+ '<a href="' + valiny.url + '">' +'Voir les d&eacute;tails' + '</a>' + '</div>').appendTo($('.mCSB_container')).addClass('new');
            setDate();
            updateScrollbar();
        }
        else if(valiny.value == null && valiny.url == null) {
            $('<div class="message new"><figure class="avatar"><img src="/opus/assets/img/logo_A.png" /></figure>' + valiny + '</a>' + '</div>').appendTo($('.mCSB_container')).addClass('new');
            setDate();
            updateScrollbar();
        }
        setDate();
